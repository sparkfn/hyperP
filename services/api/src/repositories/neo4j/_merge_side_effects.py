...
async def apply_merge_review_side_effects(
    ...
    """Redirect/close review cases affected by a merge; runs in the merge tx."""
    await tx.run(
        CLOSE_PERSON_PAIR_CASES_FOR_ABSORBED,
...
async def revert_merge_review_side_effects(
    ...
    await tx.run(REVERT_PERSON_PAIR_REDIRECTS_LEFT, merge_event_id=merge_event_id)
    await tx.run(REVERT_PERSON_PAIR_REDIRECTS_RIGHT, merge_event_id=merge_event_id)
