...
SEED_REPORTS: list[dict[str, str]] = [
    {
        ...
        "cypher_query": (
            ...
            "OPTIONAL MATCH (p)-[p_shared:IDENTIFIED_BY|LIVES_AT]-(shared)"
            "-[other_shared:IDENTIFIED_BY|LIVES_AT]-(other:Person)\n"
            "WHERE other.person_id <> p.person_id\n"
            "  AND coalesce(p_shared.is_active, true) = true\n"
            "  AND coalesce(other_shared.is_active, true) = true\n"
    ...
    },
    {
        ...
        "cypher_query": (
            ...
            "<-[identified_by:IDENTIFIED_BY]-(p:Person)\n"
            "WHERE p.status = 'active'\n"
            "  AND coalesce(identified_by.is_active, true) = true\n"
            "WITH i.normalized_value AS phone, collect(DISTINCT p.person_id) AS person_ids\n"
