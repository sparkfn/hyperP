...
def _connector(handler: Any, *, sleeper: Any = None) -> SGGovernmentBankruptcyIncrementalConnector:
    kwargs: dict[str, Any] = {
    ...
    return SGGovernmentBankruptcyIncrementalConnector(**kwargs)
...
def test_incremental_fetch_all_pages_bootstrap() -> None:
    ...
    assert page2.max_updated_at == datetime(2026, 7, 3, 12, 0, 0, tzinfo=UTC)
    assert call_count == 3
...
def test_incremental_fetch_with_updated_since_delta() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        sent_params.append(request.url.params.get("updated_since"))
        return httpx.Response(
            200,
            json={
                "items": [
                    _make_item(last_seen_at="2026-07-05T12:00:00Z"),
                ],
                "next_cursor": None,
            },
        )
...
def test_incremental_updated_at_fallback_to_last_seen_at() -> None:
    ...
    connector.open_query(updated_since=None)
    page = connector.fetch_next_page()
    assert page.max_updated_at == datetime(2026, 7, 5, 12, 0, 0, tzinfo=UTC)
...
def test_incremental_get_source_key() -> None:
    ...
    connector = _connector(handler)
    assert connector.get_source_key() == "sgbankruptcy"
...
class _Session:
    ...
    def execute_write(self, work: Any, **_kwargs: object) -> Any:
...
def test_incremental_bankruptcy_run_advances_watermark_on_drain() -> None:
    ...
    watermark = load_watermark(redis_client, "sgbankruptcy")
    assert watermark.updated_at == datetime(2026, 8, 1, 10, 0, 0, tzinfo=UTC)
    ...
    watermark_b = load_watermark(redis_client_b, "sgbankruptcy")
...
def test_incremental_timestamps_normalized_to_utc() -> None:
    ...
    assert page.max_updated_at == datetime(2026, 7, 3, 12, 0, 0, tzinfo=UTC)
