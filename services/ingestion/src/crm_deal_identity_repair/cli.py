...
def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--negative-control-limit", type=int, default=100)
    parser.add_argument("--retention-days", type=int, default=30)
    arguments = parser.parse_args(argv)
    if arguments.retention_days < 1:
        parser.error("--retention-days must be positive")
    if arguments.negative_control_limit < 1:
        parser.error("--negative-control-limit must be positive")
    return arguments
