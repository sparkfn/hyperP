...
MATCH (p)-[primary_identifier:IDENTIFIED_BY]->(pi:Identifier)
WHERE coalesce(primary_identifier.is_active, true) = true
...
WHERE coalesce(nric_identifier.is_active, true) = true
  AND ni.kind IN ['nric','nric_hash'] AND customer_nric IS NOT NULL
  AND customer_nric <> '' AND ni.value <> customer_nric
