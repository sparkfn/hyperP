...
WITH census, unit, collect(existing) AS existing_rows
WHERE size(existing_rows) <= 1
...
WITH census, unit, publication, publication.publication_id = $publication_id AS created
WITH census, unit, publication, created,
  publication.payload_digest = $payload_digest AND publication.payload_json = $payload_json
  AND publication.task_id = $task_id AND publication.task_name = $task_name AND publication.queue = $queue
  AND publication.generation = $generation AND publication.unit_kind = $unit_kind
  AND publication.sequence = $sequence AS immutable_match
FOREACH (_ IN CASE WHEN created THEN [1] ELSE [] END |
  SET unit.state = 'publishing', census.state = 'publishing', unit.updated_at = datetime(),
...
RETURN publication.publication_id AS publication_id, publication.task_id AS task_id,
  ...
  NOT immutable_match AS payload_conflict
"""
)
...
WHERE census.cancel_requested_at IS NULL AND publication.status IN ['reserved','ambiguous']
...
SET publication.status = 'ambiguous', publication.updated_at = datetime()
RETURN publication.publication_id AS publication_id
"""
)
...
WITH census, census.source_window_json IS NULL AND coalesce(census.no_source_window, false) = false AS pre_window,
  census.cancel_requested_at IS NULL AS first_request
SET census.cancel_requested_at = coalesce(census.cancel_requested_at, datetime()),
  ...
  census.state = CASE WHEN census.terminal_state = 'failed' THEN 'failed'
                      WHEN pre_window THEN 'freeze_failed' ELSE 'cancel_requested' END,
  census.terminal_state = CASE WHEN census.terminal_state = 'failed' THEN 'failed'
                               WHEN pre_window THEN 'freeze_failed' ELSE census.terminal_state END,
  census.terminal_reason = CASE WHEN census.terminal_state = 'failed' THEN census.terminal_reason
                                WHEN pre_window THEN coalesce(census.terminal_reason, 'cancelled_before_window')
                                ELSE census.terminal_reason END,
  census.terminal_at = CASE WHEN pre_window THEN coalesce(census.terminal_at, datetime()) ELSE census.terminal_at END,
  census.updated_at = datetime()
WITH census, pre_window, first_request
...
SET unit.cancel_requested_at = datetime(), unit.state = 'cancelled',
  unit.skipped_count = CASE WHEN coalesce(unit.processed_count, 0) + coalesce(unit.skipped_count, 0)
      + coalesce(unit.failed_count, 0) + coalesce(unit.no_work_count, 0) = 0 THEN 1 ELSE unit.skipped_count END,
  unit.updated_at = datetime()
WITH census, pre_window, first_request, count(unit) AS directly_cancelled
OPTIONAL MATCH (publication:StandaloneCrmChildPublication {census_id: census.census_id})
WHERE NOT pre_window AND first_request AND publication.status IN ['reserved','publishing','ambiguous']
  AND NOT EXISTS {
...
WITH census, pre_window, first_request, directly_cancelled,
...
SET retired_unit.cancel_requested_at = datetime(), retired_unit.state = 'cancelled',
  retired_unit.skipped_count = CASE WHEN coalesce(retired_unit.processed_count, 0)
      + coalesce(retired_unit.skipped_count, 0) + coalesce(retired_unit.failed_count, 0)
      + coalesce(retired_unit.no_work_count, 0) = 0 THEN 1 ELSE retired_unit.skipped_count END,
  retired_unit.updated_at = datetime()
WITH census, pre_window, first_request, directly_cancelled, retired_publications,
...
WITH census, pre_window, directly_cancelled, retired_units, retired_publications, count(fence) AS active_fences
...
FOREACH (_ IN CASE WHEN pre_window THEN [1] ELSE [] END | REMOVE scope.active_census_id)
RETURN directly_cancelled + retired_units AS child_count, retired_publications, active_fences,
  pre_window AS freeze_failed
"""
)
...
CALL {
  ...
  WHERE old.state IN ['paused_with_checkpoint','superseded']
    AND NOT EXISTS {
      MATCH (old_publication:StandaloneCrmChildPublication {census_id: census.census_id, generation: old.generation})
