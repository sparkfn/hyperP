"""Read-only source, mapping, and active-head boundary Cypher for CRM projections."""
...
MATCH (release)-[:MATERIALIZES_MAPPING_REVISION]->(revision:CrmTenantMappingRevision {
  ...
  revision_number: release.mapping_revision_number,
  manifest_digest: release.mapping_manifest_digest, state: 'prepared',
  expected_head_id: release.expected_mapping_head_id,
...
RETURN properties(release) AS release,
  [(release)-[:MATERIALIZES_SOURCE_CENSUS]->(source) | source.census_id] AS source_census_ids,
  [(release)-[:MATERIALIZES_MAPPING_REVISION]->(mapping) | mapping.revision_id]
