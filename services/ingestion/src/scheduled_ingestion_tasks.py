...
def _dispatch_active_bitrix_successor(occurrence: str) -> str | None:
    ...
    admit_configured_bitrix_control(
        get_settings(), control_instance_id
    )
    ...
    try:
        reservation_repository = CrmDealRepairControlRepository(
            reservation_graph
        )
        ...
        if any(
            entry.stream_key == "crm_activities" and entry.executes
        ...
        categories = tuple(
            ...
            .bitrix_openlines.included_crm_category_ids
        )
        executable = manifest.operational_entries
        refresh_deals = any(
            entry.stream_key == "crm_deals"
            and entry.replay_mode != "fixed_keyset"
        ...
        if refresh_deals:
            ...
            try:
                if refresh_deals:
                    upper_deal_id = freeze_deal_upper_id(
                        source, categories
                    )
            finally:
                source.close()
...
def _occurrence(
    group: ScheduledIngestionGroup, now: datetime
) -> ScheduledOccurrence:
...
def _drive_group(
    group_key: str, incremental: bool, now: datetime
) -> ScheduledGroupDispatchSummary:
    ...
    if group.key == "bitrix_chat":
        successor = _dispatch_active_bitrix_successor(
            now.date().isoformat()
        )
        if successor is not None:
            return _summary(
                "queued", group.key, incremental, successor
            )
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name=(
        "src.scheduled_ingestion_tasks"
        ".reconcile_ingestion_group_task"
    ),
    bind=True,
    max_retries=0,
)
...
def dispatch_scheduled_maintenance_task(
    ...
    if kind == "knows" and phase not in {
        ...
        raise ValueError(
            "KNOWS maintenance requires a supported phase"
        )
    if kind == "lifecycle" and phase is not None:
        raise ValueError(
            "lifecycle maintenance does not accept a phase"
        )
    if not get_ingestion_config().scheduled_ingestion.enabled:
        return "disabled"
    now = _utc_now()
    ...
    group = (
        None
        if weekday is None
        else scheduled_ingestion_group_for_weekday(weekday)
    )
    if group is None:
    ...
    materialize_knows_task.apply_async(
        args=(phase,), queue=LIFECYCLE_QUEUE
    )
    return "published"
