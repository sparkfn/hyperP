...
def retire_source_evidence(
    ...
    *,
    ...
    return client.execute_write(
        lambda tx: retire_source_evidence_in_transaction(
            tx,
            source_system,
            source_record_id,
            retired_at,
...
def retire_source_evidence_in_transaction(
    ...
    *,
    ...
    """Retire active evidence for one source record in a caller-owned transaction.

    Identity-link revisions, active-evidence retirement, and CRM-count
    recomputation stay in the caller's transaction so a bounded unit commits
    them atomically with its own checkpoint.
    """
