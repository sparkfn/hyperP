...
class IngestPipeline:
    ...
    def _require_client(self) -> Neo4jClient:
        client = self._client
        if client is None:
            raise RuntimeError("transaction-only pipeline cannot open a graph session")
        return client
    ...
    def ingest_in_transaction(
        ...
        *,
        ...
        """Apply one identity or relationship envelope in an existing transaction.

        The bounded-ingestion writer owns the transaction containing its receipt
        and checkpoint. This hook reuses the canonical lifecycle and matching
        path rather than opening a nested transaction.
        """
