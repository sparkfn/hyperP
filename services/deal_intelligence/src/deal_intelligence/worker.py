"""Disabled worker entry point; no task registry is loaded or executed."""
...
def run_one_cycle(writer: ProcessHeartbeatWriter | None = None) -> IdleWorker:
    """Write exactly one worker readiness heartbeat and execute no task."""
    active_writer = writer or HeartbeatWriter(get_database())
    active_writer.write("worker")
    return run()
...
def run_idle_loop(stop_event: Event, writer: ProcessHeartbeatWriter | None = None) -> None:
    ...
    while not stop_event.is_set():
        try:
            ...
            logger.exception("Deal Intelligence worker heartbeat failed")
