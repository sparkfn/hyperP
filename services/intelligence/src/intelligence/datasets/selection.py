...
from intelligence.datasets.models import instant_key
...
def _eligible(record: DealReference, cutoff: str) -> bool:
    ...
    try:
        cutoff_value = instant_key(cutoff, "cutoff")
        return all(instant_key(value, "deal temporal evidence") <= cutoff_value for value in values)
    except ValueError:
        return False
...
def _order(record: DealReference) -> tuple[object, ...]:
    ...
    return (
        instant_key(effective, "source effective"),
        instant_key(observed, "observed"),
        instant_key(available, "available"),
