...
function buildTheme(palette: PaletteOptions, tableHeadBg: string, borderBase: string): Theme {
  return createTheme({
    cssVariables: true,
    palette,
    ...
    components: {
      ...
      MuiPaper: {
        ...
        styleOverrides: { outlined: { borderColor: `rgba(${borderBase},0.12)` } },
      },
      ...
      MuiDivider: { styleOverrides: { root: { borderColor: `rgba(${borderBase},0.08)` } } },
      MuiCard: {
        defaultProps: { elevation: 0 },
        styleOverrides: { root: { border: `1px solid rgba(${borderBase},0.12)` } },
      },
    },
  });
}
