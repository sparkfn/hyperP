...
export function SourceRecordDetails({ record }: { record: PersonSourceRecord }): ReactElement {
    ...
    <Stack spacing={3}>
      <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
        ...
        <Chip
          ...
          size="small"
          color={record.record_type === "conversation" ? "warning" : "default"}
        />
      ...
      <Grid container spacing={2}>
        ...
        <PayloadMeta label="Extraction confidence" value={record.extraction_confidence?.toFixed(2) ?? "—"} />
      </Grid>
      <Divider />
      {payload === null ? (
...
export function sourceRecordEntityLabel(record: PersonSourceRecord): string {
  return entityLabel(record);
}
...
function IdentifierSection({ identifiers }: { identifiers: SourceRecordIdentifierPayload[] }): ReactElement {
  return (
    <Box>
      <Typography variant="subtitle2" gutterBottom>
        Identifiers
...
function AttributeSection({ attributes }: { attributes: SourceRecordAttributePayload[] }): ReactElement {
  return (
    <Box>
      <Typography variant="subtitle2" gutterBottom>
        Attributes
